    ...
    <div className="min-h-screen relative bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-4 font-sans">
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="w-full max-w-md flex flex-col">
          ...
          {/* --- CARD CONTAINER --- */}
          {/* Fixed height (80vh). Flex column. Content flows Top -> Down. */}
          <div className="relative backdrop-blur-xl bg-white/10 border border-white/15 shadow-2xl rounded-3xl overflow-hidden flex flex-col h-[80vh] min-h-[600px] max-h-[800px]">
            ...
            {/* 2. CONTENT (Starts immediately after Header) */}
            {/* flex-1 ensures it fills the space, but justify-start (default) keeps content at the top */}
            <div 
              className={[
                "relative z-10 w-full px-6 flex-1 overflow-y-auto",
                // Remove scrollbar visuals
                "[scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden",
                // Add bottom padding so content doesn't touch the edge (or nav)
            ...
            {/* 3. BOTTOM NAV (Fixed at bottom, only if passed) */}
            {isAppLayout && (
