...
function AuthCard({ title, children, onBack, topRight, bottom }) {
  ...
  return (
    // ✅ Lock to screen (no page scroll), stable iOS height, safe-area padding
    <div
      className="fixed inset-0 overflow-hidden relative bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 font-sans"
      style={{
    ...
    >
      {/* ✅ Stable height so navbar doesn't jump when Safari bars show/hide */}
      <div className="h-[100svh] w-full overflow-hidden relative">
        {/* Top Right Slot (Logout) */}
        {topRight ? (
          <div className="absolute top-0 right-0 z-20">{topRight}</div>
        ) : null}

        {/* Center container MUST NOT create page scroll */}
        <div className="h-full flex items-center justify-center">
          <div className="w-full max-w-md h-full max-h-[900px] flex flex-col min-h-0">
